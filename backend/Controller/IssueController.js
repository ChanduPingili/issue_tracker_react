...
async function IssueSave(req, res) {
    try {
        ...
        const employee = await EmployeeModel.findOne({ taskCount: { $lt: 15} ,empOrgId:issueOrgId,empServiceId:issueServiceId});
        if (!employee) {
        ...
        const newIssueId =7000 + (await issueModel.countDocuments()*7) + 1;
