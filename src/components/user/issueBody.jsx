...
const IssueBody = ({ issue, status }) => {
	...
	const handleDelete = async () => {
		...
		await axios.delete(`http://localhost:2000/api/issue:${issue.issueId}`);
	};
	...
	const handleEditIssue = async () => {
		...
		await axios.put(
			`http://localhost:2000/api/issue:${issue.issueId}`,
			newIssue
		);
	};
	...
	return (
		<Card className="text-center">
			...
			<Card.Footer className="text-muted">
				...
				></input>
				<hr />
				Status : {status.statusDescription} | Status Id : {issue.status} |
